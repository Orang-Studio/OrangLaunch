...
namespace OrangLauncher
{
    public class InstanceIconConverter : IValueConverter
    {
        public object? Convert(object value, Type targetType, object parameter, CultureInfo culture)
        {
            if (value is MinecraftInstance instance)
            {
                ...
                string iconName = "minecraft-green.png"; 
                if (!string.IsNullOrEmpty(instance.ModLoader) && 
                    ...
                    !string.IsNullOrWhiteSpace(instance.ModLoader))
                {
                    iconName = "minecraft-blue.png";
                }
                string[] checkPaths =
                [
    ...
    public partial class MainWindow : Window, INotifyPropertyChanged
    {
        ...
        public string LocalizedAboutVersion => LocalizationManager.GetString("SETTINGS_ABOUT_VERSION", "Version: 6.0.0");
        ...
        private void LoadInstanceIcon(MinecraftInstance? instance)
        {
            try
            {
                if (instance != null && !string.IsNullOrEmpty(instance.Icon) && File.Exists(instance.Icon))
                {
                    var bitmap = new BitmapImage();
                    bitmap.BeginInit();
                    bitmap.CacheOption = BitmapCacheOption.OnLoad;
                ...
                string iconName = "minecraft-green.png";
                if (instance != null && !string.IsNullOrEmpty(instance.ModLoader) &&
                    !instance.ModLoader.Equals("vanilla", StringComparison.OrdinalIgnoreCase) &&
                    !instance.ModLoader.Equals("none", StringComparison.OrdinalIgnoreCase))
                {
                    iconName = "minecraft-blue.png";
        ...
        private async void ProfileModLoaderComboBox_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            if (ProfileModLoaderComboBox.SelectedItem is ComboBoxItem selected)
            {
                ...
                var mcVersion = ProfileVersionComboBox.Text;
        ...
        private void DownloadResourcePacksButton_Click(object sender, RoutedEventArgs e)
        {
            GetCurrentInstanceVersionInfo(out string version, out _);
            string path = GetCurrentResourcePacksPath(false);
            new DownloadWindow(path, "resourcepack", version).ShowDialog();
            LoadResourcePacks();
        }
        ...
        private void DownloadShaderPacksButton_Click(object sender, RoutedEventArgs e)
        {
            GetCurrentInstanceVersionInfo(out string version, out _);
            string path = GetCurrentResourcePacksPath(true);
            new DownloadWindow(path, "shader", version).ShowDialog();
            LoadShaderPacks();
        }
        ...
        private void DownloadModsButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (loader == "vanilla" || loader == "none")
            {
                MessageBox.Show("Please select a mod loader (Fabric/Forge) in your profile settings before downloading mods.", "No Loader Selected", MessageBoxButton.OK, MessageBoxImage.Warning);
                return;
            }
            string path = GetCurrentModsPath();
            new DownloadWindow(path, "mod", version, loader).ShowDialog();
            LoadMods();
        }
        private void LogMessage(string message) { Dispatcher.Invoke(() => { LogTextBox.AppendText($"[{DateTime.Now:HH:mm:ss}] {message}\n"); LogTextBox.ScrollToEnd(); }); }
        ...
        private async void ImportModpackButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (openFileDialog.ShowDialog() == true)
            {
                try
                {
                    ...
                    var (success, message, instanceId) = await importer.ImportMrPackAsync(openFileDialog.FileName);
                    StatusText = message;
        ...
        private async void LoadVersionsAsync()
        {
            try
            {
                ...
                {
                    if (!versionIds.Contains(v.Id) &&
                        !v.Id.ToLower().Contains("fabric") &&
                        !v.Id.ToLower().Contains("forge") &&
                        !v.Id.ToLower().Contains("quilt") &&
                        !v.Id.ToLower().Contains("neoforge") &&
                        !v.Id.ToLower().Contains("liteloader"))
                    {
                        versionIds.Add(v.Id);
                    }
                }
                try 
                {
                    string mcVersionsDir = System.IO.Path.Combine(PlatformPaths.GetMinecraftDir(), "versions");
                    if (Directory.Exists(mcVersionsDir))
                    {
                        var localDirs = Directory.GetDirectories(mcVersionsDir);
                        foreach (var dir in localDirs) 
                        { 
                            var dirName = System.IO.Path.GetFileName(dir);
                            if (dirName != null && 
                                !dirName.ToLower().Contains("fabric") &&
                                !dirName.ToLower().Contains("forge") &&
                                !dirName.ToLower().Contains("quilt") &&
                                !dirName.ToLower().Contains("neoforge") &&
                                !dirName.ToLower().Contains("liteloader") &&
                                !versionIds.Contains(dirName))
                            {
                                versionIds.Add(dirName);
                            }
                        }
                    }
                }
                catch {}
                var fallbackVersions = new[] { "1.21.11", "1.20.4", "1.20.1", "1.19.4", "1.19.2", "1.18.2", "1.16.5", "1.12.2", "1.8.9", "1.7.10" };
                foreach (var v in fallbackVersions)
                {
                    if (!versionIds.Contains(v))
                        versionIds.Add(v);
                }
        ...
        private void SaveSettings()
        {
            try
            {
                ...
                var config = new Dictionary<string, object>
                {
                    ...
                    ["theme"] = (ThemeComboBox.SelectedItem as ComboBoxItem)?.Tag?.ToString() ?? "arc",
                    ...
                    ["newsBrowser"] = (NewsBrowserComboBox.SelectedItem as ComboBoxItem)?.Tag?.ToString() ?? "webview2",
        ...
        private async Task InitializeNewsBrowser()
        {
            try
            {
                ...
                {
                    LogMessage("Skipping WebView2 initialization (IE selected)");
                }
            }
            catch (Exception ex)
            {
        ...
        private async Task LoadCurrentAccountSkinAsync()
        {
            ...
            try
            {
                ...
                if (skinInfo != null)
                {
                    var headImage = await SkinManager.GetHeadAvatarAsync(skinInfo.Uuid, 64);
                    var bodyImage = await SkinManager.GetBodyRenderAsync(skinInfo.Uuid, 4);
                    if (headImage == null && skinInfo.HeadImage != null)
                    {
                        headImage = skinInfo.HeadImage;
                    }
                    await Dispatcher.InvokeAsync(() => 
                    {
                        SkinHeadImage.Source = headImage;
                        SkinBodyImage.Source = bodyImage ?? skinInfo.SkinImage;
                        if (skinInfo.IsSlim)
                        {

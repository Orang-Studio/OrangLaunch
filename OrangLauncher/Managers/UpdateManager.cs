    ...
    public static class UpdateManager
    {
        ...
        public static async Task<UpdateInfo> CheckForUpdatesAsync()
        {
            ...
            try
            {
                ...
                client.DefaultRequestHeaders.Add("User-Agent", "OrangLauncher/5.0");
                ...
                result.NewVersion = tagName.TrimStart('v', 'V');
                result.ReleaseNotes = body;
                result.ReleaseName = releaseName;
                if (Version.TryParse(result.NewVersion.Split('-')[0], out var newVersion))
                {
                    result.UpdateAvailable = newVersion > CurrentVersion && !prerelease;
                }
        ...
        public static async Task<bool> DownloadAndInstallUpdateAsync(UpdateInfo updateInfo)
        {
            if (!updateInfo.UpdateAvailable || string.IsNullOrEmpty(updateInfo.DownloadUrl))
                return false;
            if (updateInfo.DownloadUrl.EndsWith(".msi", StringComparison.OrdinalIgnoreCase) ||
                updateInfo.DownloadUrl.EndsWith(".exe", StringComparison.OrdinalIgnoreCase))
            {
                try
                {

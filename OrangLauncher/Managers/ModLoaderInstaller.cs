    ...
    public static class ModLoaderInstaller
    {
        ...
        public static async Task<List<ModLoaderVersion>> GetAvailableLoadersAsync(string minecraftVersion)
        {
            ...
            try
            {
                ...
                var json = JsonDocument.Parse(response);
                bool firstFabric = true;
                foreach (var item in json.RootElement.EnumerateArray().Take(10))
                {
            ...
            try
            {
                ...
                if (mcParts.Length >= 2)
                {
                    ...
                    if (minor >= 20 && (minor > 20 || patch >= 1))
                    {
                        ...
                        var neoResponse = await _httpClient.GetStringAsync("https://maven.neoforged.net/api/maven/versions/releases/net/neoforged/neoforge");
                        var neoJson = JsonDocument.Parse(neoResponse);
                        if (neoJson.RootElement.TryGetProperty("versions", out var versionArray))
                        {

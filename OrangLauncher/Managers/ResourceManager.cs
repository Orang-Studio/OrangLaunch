...
namespace OrangLauncher.Managers
{
    public class ResourceManager
    {
        ...
        public void AddPacks(string path, bool isShader)
        {
            Directory.CreateDirectory(path);
            var dialog = new OpenFileDialog
            {
                Title = isShader ? "Select Shader Packs" : "Select Resource Packs",
                Filter = "Pack Files (*.zip)|*.zip|All Files (*.*)|*.*",
                Multiselect = true
            };
            if (dialog.ShowDialog() == true)
            {
                ...
                {
                    var destPath = Path.Combine(path, Path.GetFileName(file));

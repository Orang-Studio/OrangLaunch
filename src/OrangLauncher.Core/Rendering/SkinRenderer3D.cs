...
namespace OrangLauncher.Rendering
{
    ...
    public static class SkinRenderer3D
    {
        ...
        private static List<Face> BuildModel(bool slim, bool overlay, bool legacy)
        {
            ...
            if (legacy)
                AddBox(f, new Vector3(armXOff, 0, 0), armW, 12, 4, 40, 16, armW, 12, 4, true, 0); // left arm
            else
            ...
            if (legacy)
                AddBox(f, new Vector3(2, -12, 0), 4, 12, 4, 0, 16, 4, 12, 4, true, 0);    // left leg
            else
                AddBox(f, new Vector3(2, -12, 0), 4, 12, 4, 16, 48, 4, 12, 4, false, 0);
            if (overlay)
            {
                AddBox(f, new Vector3(0, 10, 0), 8, 8, 8, 32, 0, 8, 8, 8, false, 0.5f);   // hat
                if (!legacy)
                {
                                         ...
                                         in Face face, SkinTexture tex, float shade)
        {
            ...
            for (int y = minY; y <= maxY; y++)
            {
                for (int x = minX; x <= maxX; x++)
                {
                    ...
                    if (alpha < 8) continue; // transparent texel
                    zbuf[zi] = z;
                    int pi = zi * 4;

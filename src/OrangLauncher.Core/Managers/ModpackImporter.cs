    ...
    public class ModpackImporter
    {
        ...
        public ModpackImporter()
        {
            _httpClient.DefaultRequestHeaders.Add("User-Agent",
                $"OrangLauncher/{UpdateManager.GetCurrentVersionString().TrimStart('v')} (github.com/Orang-Studio/OrangLaunch)");
        }
            ...
            try
            {
                ...
                foreach (var entry in archive.Entries)
                {
                    if (entry.FullName.StartsWith("overrides/") && !string.IsNullOrEmpty(entry.Name))
                    {
                        ...
                        var destPath = ResolveInsideDir(instance.MinecraftDir, relativePath);
                ...
                if (root.TryGetProperty("files", out var filesElement) && filesElement.ValueKind == JsonValueKind.Array)
                {
                    ...
                    foreach (var fileElem in fileList)
                    {
                        ...
                        var rel = pathStr.StartsWith("overrides/", StringComparison.OrdinalIgnoreCase)
                            ? pathStr.Substring("overrides/".Length)
                            : pathStr;
                        var destPath = ResolveInsideDir(instance.MinecraftDir, rel);
                        if (destPath == null)
                        {
                            continue;
                        }
        ...
        private static string? ResolveInsideDir(string baseDir, string relativePath)
        {
            ...
            var destFull = Path.GetFullPath(Path.Combine(baseFull, relativePath));
            return destFull.StartsWith(baseFull, StringComparison.OrdinalIgnoreCase) ? destFull : null;
        }

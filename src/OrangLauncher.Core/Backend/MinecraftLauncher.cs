...
namespace OrangLauncher.Backend
{
    ...
    public class MinecraftLauncher
    {
        ...
        private List<string> BuildGameArguments(VersionMeta meta, string versionId, MLaunchOption options, string nativesDir, string effectiveGameDir)
        {
            ...
            var activeFeatures = new Dictionary<string, bool>
            {
                ["has_custom_resolution"] = options.ScreenWidth.HasValue && options.ScreenHeight.HasValue,
                ["is_demo_user"] = false,
            ...
            if (meta.Arguments?.Game != null)
            {
                foreach (var arg in meta.Arguments.Game)
                {
                    if (arg is JsonElement elem)
                    {
                        ...
                        {
                            if (EvaluateRuleArg(elem, out var conditionalGameValues, activeFeatures))
                                foreach (var val in conditionalGameValues)
                                    args.Add(ReplaceTemplates(val, replacements));
                        }
                    }
                }
            }
        ...
        private bool EvaluateRuleArg(JsonElement elem, out List<string> values,
            Dictionary<string, bool>? activeFeatures = null)
        {
            values = new List<string>();
            try
            {
                if (!elem.TryGetProperty("rules", out var rules))
                {
                    if (elem.TryGetProperty("value", out var noRuleVal))
                    {
                        if (noRuleVal.ValueKind == JsonValueKind.String)
                            values.Add(noRuleVal.GetString()!);
                        else if (noRuleVal.ValueKind == JsonValueKind.Array)
                            foreach (var v in noRuleVal.EnumerateArray())
                ...
                foreach (var rule in rules.EnumerateArray())
                {
                    ...
                    if (rule.TryGetProperty("os", out var os))
                    {
                        ...
                        if (matches && os.TryGetProperty("arch", out var arch))
                        {
                            var currentArch = RuntimeInformation.OSArchitecture == Architecture.X64 ? "x86" : "x86_32";
                            matches = matches && arch.GetString() == currentArch;
                        }
                    }
                    if (matches && rule.TryGetProperty("features", out var features) && activeFeatures != null)
                    {
                        foreach (var feature in features.EnumerateObject())
                        {
                            var featureName = feature.Name;
                            var requiredValue = feature.Value.GetBoolean();
                            if (activeFeatures.TryGetValue(featureName, out var actualValue))
                            {
                    ...
                    if (action == "allow")
                    {
                        if (rule.TryGetProperty("os", out _) || rule.TryGetProperty("features", out _))
                        {
                            if (matches)
                                allowed = true;
                        }
        ...
        {
            var managedJava = Managers.JavaManager.GetJavaPath(majorVersion);
            if (!string.IsNullOrEmpty(managedJava) && File.Exists(managedJava))
                return managedJava;
            try
            {
                var javaInstallDir = Managers.JavaManager.GetJavaInstallDir();
                if (Directory.Exists(javaInstallDir))
                {
                    ...
                    {
                        managedJava = Managers.JavaManager.GetJavaPath(ver);
                        if (!string.IsNullOrEmpty(managedJava) && File.Exists(managedJava))
                            return managedJava;
                    }
                }

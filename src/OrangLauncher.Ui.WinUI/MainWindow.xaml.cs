...
namespace OrangLauncher
{
    ...
    public sealed partial class MainWindow : Window, INotifyPropertyChanged
    {
        ...
        public MainWindow()
        {
            ...
            if (appWindow != null)
            {
                appWindow.Resize(new Windows.Graphics.SizeInt32(1300, 820));
        ...
        private async void InitializeApplication()
        {
            try
            {
                ...
                await ApplyNewsBrowserAsync(GetNewsBrowserSetting());
        ...
        private void ApplyLocalization()
        {
            try
            {
                ...
                if (MainNavView.MenuItems.Count > 5 && MainNavView.MenuItems[5] is NavigationViewItem nav5)
                    nav5.Content = LocalizationManager.GetString("CONTENT_TAB", "Content");
                if (MainNavView.MenuItems.Count > 6 && MainNavView.MenuItems[6] is NavigationViewItem nav6)
                    nav6.Content = LocalizationManager.GetString("SETTINGS", "Settings");
                ...
                {
                    ((ComboBoxItem)UiStyleComboBox.Items[0]).Content = LocalizationManager.GetString("SETTINGS_UI_STYLE_MODERN", "Modern (Windows 11 style)");
                    ((ComboBoxItem)UiStyleComboBox.Items[1]).Content = LocalizationManager.GetString("SETTINGS_UI_STYLE_CLASSIC", "Classic (Windows 10 style)");
                }
                ToolTipService.SetToolTip(SkinBodyViewer, LocalizationManager.GetString("SKIN_DRAG_HINT", "Drag to rotate"));
                ...
                SettingsNewsBrowserLabel.Text = LocalizationManager.GetString("SETTINGS_BROWSER_TITLE", "News Renderer");
                SettingsNewsBrowserDesc.Text = LocalizationManager.GetString("SETTINGS_BROWSER_DESC", "WebView2 shows the full news page; the native renderer works without WebView2.");
                ...
                AboutVersionText.Text = UpdateManager.GetAboutVersionText(
                ...
                NewsFallbackText.Text = LocalizationManager.GetString("NEWS_FALLBACK", "News cannot be shown because WebView2 is not available.");
                NewsOpenBrowserButton.Content = LocalizationManager.GetString("NEWS_OPEN_BROWSER", "Open news in browser");
        ...
        private void MainNavView_SelectionChanged(NavigationView sender, NavigationViewSelectionChangedEventArgs args)
        {
            if (args.SelectedItem is NavigationViewItem item && item.Tag is string tag)
            {
                ...
                ContentPanel.Visibility = tag == "content" ? Visibility.Visible : Visibility.Collapsed;
                SettingsPanel.Visibility = tag == "settings" ? Visibility.Visible : Visibility.Collapsed;
                if (tag == "resources") { LoadResourcePacks(); LoadShaderPacks(); LoadMods(); }
                else if (tag == "servers") LoadServerList();
                else if (tag == "content") InitContentPage();
            }
        }
        ...
        private string? _quickPlayIp;
        private int? _quickPlayPort;
        private void QuickPlayServerButton_Click(object sender, RoutedEventArgs e)
        {
            if ((sender as FrameworkElement)?.Tag is not ViewModels.ServerViewModel vm) return;
            if (_currentProcess != null) return; // game already running
            var parts = vm.Ip.Split(':');
            _quickPlayIp = parts[0];
            _quickPlayPort = parts.Length > 1 && int.TryParse(parts[1], out var p) ? p : null;
        ...
        private async void PlayButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                var quickPlayIp = _quickPlayIp; var quickPlayPort = _quickPlayPort;
                _quickPlayIp = null; _quickPlayPort = null;
                if (!string.IsNullOrEmpty(quickPlayIp)) LogMessage($"Quick play: {quickPlayIp}:{quickPlayPort ?? 25565}");
                _currentProcess = await GameProfileManager.Instance.LaunchGame(version, maxRam, javaArgs, session, modLoader, gameDir, quickPlayIp, quickPlayPort, performanceTier, useDiscreteGpu);
        ...
        private const string NewsUrl = "https://oranges.lt/launcher.html";
        private string GetNewsBrowserSetting()
        {
            try
            {
                ...
                if (File.Exists(configPath))
                {
                    ...
                    if (config != null && config.TryGetValue("newsBrowser", out var b))
                        return b.GetString() ?? "webview2";
                }
            }
            catch { }
            return "webview2";
        }
        private bool _newsBrowserSyncing;
        private void NewsBrowserComboBox_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            if (_newsBrowserSyncing) return;
            if (NewsBrowserComboBox.SelectedItem is ComboBoxItem item && item.Tag is string browser)
            {
                SaveSettings();
                _ = ApplyNewsBrowserAsync(browser);
            }
        }
        private async Task ApplyNewsBrowserAsync(string browser)
        {
            ...
            try
            {
                foreach (ComboBoxItem item in NewsBrowserComboBox.Items)
                    if (item.Tag?.ToString() == browser) { NewsBrowserComboBox.SelectedItem = item; break; }
            }
            finally { _newsBrowserSyncing = false; }
            if (browser == "native")
            {
                NewsWebView.Visibility = Visibility.Collapsed;
                NewsFallbackPanel.Visibility = Visibility.Collapsed;
                NewsNativePanel.Visibility = Visibility.Visible;
                await LoadNativeNewsAsync();
                return;
            }
            try
            {
                ...
                await NewsWebView.EnsureCoreWebView2Async(env);
                NewsWebView.Visibility = Visibility.Visible;
                NewsFallbackPanel.Visibility = Visibility.Collapsed;
                NewsWebView.CoreWebView2.Navigate(NewsUrl);
        ...
        private async Task LoadNativeNewsAsync()
        {
            ...
            try
            {
                ...
                var html = await _newsHttp.GetStringAsync(NewsUrl);
                NewsNativeStack.Children.Clear();
        ...
        private IEnumerable<UIElement> BuildNewsBlocks(string html)
        {
            ...
            foreach (System.Text.RegularExpressions.Match m in matches)
            {
                ...
                var text = System.Net.WebUtility.HtmlDecode(
                    System.Text.RegularExpressions.Regex.Replace(m.Groups["body"].Value, "<[^>]+>", " "));
                ...
                switch (tag)
                {
                    ...
                    case "h2":
                        if (!first)
                            blocks.Add(new Border
                            {
                                ...
                                Background = Application.Current.Resources["DividerStrokeColorDefaultBrush"] as Microsoft.UI.Xaml.Media.Brush
                            });
                        blocks.Add(new TextBlock
                        {
                            Text = text, FontSize = 18, FontWeight = Microsoft.UI.Text.FontWeights.Bold,
                    ...
                    case "h6":
                        blocks.Add(new TextBlock
                        {
                            Text = text, FontSize = 14.5, FontWeight = Microsoft.UI.Text.FontWeights.SemiBold,
                            TextWrapping = TextWrapping.Wrap, IsTextSelectionEnabled = true,
                            Foreground = Application.Current.Resources["AccentTextFillColorPrimaryBrush"] as Microsoft.UI.Xaml.Media.Brush,
                    ...
                    case "li":
                        blocks.Add(new TextBlock
                        {
                            Text = "•  " + text, FontSize = 13, Opacity = 0.9,
                            TextWrapping = TextWrapping.Wrap, IsTextSelectionEnabled = true,
        ...
        private void NewsOpenBrowserButton_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                System.Diagnostics.Process.Start(new System.Diagnostics.ProcessStartInfo
                {
                    FileName = "https://oranges.lt/launcher.html",
        ...
        private async void ProfileModLoaderComboBox_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            if (ProfileModLoaderComboBox.SelectedItem is ComboBoxItem selected)
            {
                ...
                if (loaderType == "optifine")
                {
                    try
                    {
                        var mc = ProfileVersionComboBox.SelectedItem?.ToString() ?? "";
                        var builds = await OptiFineClient.GetVersionsForAsync(mc);
                        ProfileModLoaderVersionComboBox.ItemsSource = builds.Count > 0
                            ? builds.Select(b => b.Edition).ToList()
                            : (object)new[] { "Not available for this MC version" };
                    }
                    catch { ProfileModLoaderVersionComboBox.ItemsSource = new[] { "Latest" }; }
                    ProfileModLoaderVersionComboBox.SelectedIndex = 0;
                    return;
                }
                try
        ...
        private async void ImportModpackButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            var file = await picker.PickSingleFileAsync();
            if (file != null) await ImportMrpackFileAsync(file.Path);
        }
                    ...
                    if (config != null)
                    {
                        ...
                        if (config.TryGetValue("theme", out var theme))
                        {
                            ...
                            foreach (ComboBoxItem item in ThemeComboBox.Items)
                                if (item.Tag?.ToString() == themeName) { ThemeComboBox.SelectedItem = item; matched = true; break; }
                        ...
                        var newsBrowser = "webview2";
                        if (config.TryGetValue("newsBrowser", out var nb))
                            newsBrowser = nb.GetString() ?? "webview2";
                        foreach (ComboBoxItem item in NewsBrowserComboBox.Items)
                            if (item.Tag?.ToString() == newsBrowser) { NewsBrowserComboBox.SelectedItem = item; break; }
                        if (NewsBrowserComboBox.SelectedItem == null) NewsBrowserComboBox.SelectedIndex = 0;
        ...
        private void SaveSettings()
        {
            try
            {
                ...
                config["theme"] = (ThemeComboBox.SelectedItem as ComboBoxItem)?.Tag?.ToString() ?? "dark";
        ...
        private void UiStyleComboBox_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            ...
            if (UiStyleComboBox.SelectedItem is ComboBoxItem item && item.Tag is string ui && ui != UiModeManager.WinUi)
            {
            ...
            try
            {
                ...
                if (skinInfo != null)
                {
                    ...
                    if (skinBytes != null)
                    {
                        try
                        {
                            rendered3d = SkinBodyViewer.LoadSkin(skinBytes, skinInfo.IsSlim);
                            byte[]? capeBytes = string.IsNullOrEmpty(skinInfo.CapeUrl) ? null : await SkinManager.GetSkinBytesAsync(skinInfo.CapeUrl);
                            SkinBodyViewer.LoadCape(capeBytes);
                        }
                        catch (Exception rex)
                        {
                            LogMessage($"3D skin render failed: {rex.Message}");
                        }
                    ...
                    if (!rendered3d && !string.IsNullOrEmpty(skinInfo.SkinUrl))
                        frontComposite = await SkinManager.GetFrontCompositeAsync(skinInfo.SkinUrl, 160);
                    ...
                    DispatcherQueue.TryEnqueue(() =>
                    {
                        ...
                        else if (_skinFullImg != null)
                        {
                            _skinFullImg.Visibility = Visibility.Collapsed;
                        }
        ...
        private void LogMessage(string message)
        {
            ...
            DispatcherQueue.TryEnqueue(Microsoft.UI.Dispatching.DispatcherQueuePriority.Low, () =>
            {
                ...
                var text = LogTextBox.Text + chunk;
                if (text.Length > 400_000) text = "[...older log trimmed...]\n" + text[^300_000..];
        ...
        private async Task ShowMessageAsync(string message, string title = "")
        {
            try
            {
                if (Content?.XamlRoot == null) { LogMessage($"{title}: {message}"); return; }
                var dialog = new ContentDialog
                {

...
namespace OrangLauncher
{
    ...
    public sealed partial class UpdateWindow : Window
    {
        ...
        public UpdateWindow(string installPath, string projectType, string gameVersion, string modLoader = "")
        {
            ...
            AppWindow.GetFromWindowId(windowId)?.Resize(new Windows.Graphics.SizeInt32(600, 450));
            if (Content is FrameworkElement fe)
                fe.Loaded += async (s, e) => await CheckForUpdatesAsync();
        }
        ...
        private async void RecheckButton_Click(object sender, RoutedEventArgs e) => await CheckForUpdatesAsync();
        private async Task CheckForUpdatesAsync()
        {
            ...
            try
            {
                if (!Directory.Exists(_installPath)) { TitleTextBlock.Text = "No files found"; return; }
                var files = Directory.GetFiles(_installPath, "*.jar").Concat(Directory.GetFiles(_installPath, "*.zip")).ToList();
                if (files.Count == 0) { TitleTextBlock.Text = "No files found"; return; }
                ...
                var hashesJson = JsonSerializer.Serialize(new { hashes, algorithm = "sha512" });
                ...
                for (int i = 0; i < files.Count; i++)
                {
                    ...
                    if (versionDataDict.TryGetValue(hash, out var version))
                    {
                        ...
                        if (latestInfo != null && !string.Equals(latestInfo.VersionNumber, currentVersion, StringComparison.OrdinalIgnoreCase))
                        {
                            ...
                            {
                                FilePath = file, Name = ExtractModName(fileName),
                                CurrentVersion = currentVersion, LatestVersion = latestInfo.VersionNumber,
                                ProjectId = projectId, DownloadUrl = latestInfo.DownloadUrl,
                                NewFileName = latestInfo.FileName, FileHash = hash
                            });
                        }
                    }
                    else
                    {
                        ...
                        if (projectInfo != null)
                        {
                            ...
                            if (latestInfo != null)
                            {
                                _updates.Add(new UpdateItem
                                {
                                    FilePath = file, Name = projectInfo.Name ?? ExtractModName(fileName),
                                    CurrentVersion = "Unknown", LatestVersion = latestInfo.VersionNumber,
                                    ProjectId = projectInfo.ProjectId, DownloadUrl = latestInfo.DownloadUrl,
                                    NewFileName = latestInfo.FileName, FileHash = hash
                                });
                            }
                        }
            ...
            catch (Exception ex) { await ShowMessageAsync($"Error checking for updates: {ex.Message}", "Error"); }
            finally
            {
        ...
        private class ProjectSearchResult { public string? ProjectId { get; set; } public string? Name { get; set; } }
        private async Task<ProjectSearchResult?> TryFindProjectByFilenameAsync(HttpClient client, string fileName)
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(_gameVersion)) facets.Add($"[\"versions:{_gameVersion}\"]");
                ...
                foreach (var hit in json.RootElement.GetProperty("hits").EnumerateArray())
                {
                    ...
                    if (title != null && (fileName.Contains(title, StringComparison.OrdinalIgnoreCase) ||
                        fileName.Contains(slug ?? "", StringComparison.OrdinalIgnoreCase) ||
                        title.Contains(searchName, StringComparison.OrdinalIgnoreCase)))
                        return new ProjectSearchResult { ProjectId = hit.GetProperty("project_id").GetString(), Name = title };
                }
            }
            catch { }
            return null;
        }
        private class LatestVersionInfo { public string? VersionNumber { get; set; } public string? DownloadUrl { get; set; } public string? FileName { get; set; } }
                ...
                foreach (var version in OrderByStability(candidates))
                {
                    ...
                    foreach (var file in files.EnumerateArray())
                        if (file.TryGetProperty("primary", out var primary) && primary.GetBoolean())
                            return new LatestVersionInfo { VersionNumber = version.GetProperty("version_number").GetString(), DownloadUrl = file.GetProperty("url").GetString(), FileName = file.GetProperty("filename").GetString() };
                    var firstFile = files.EnumerateArray().FirstOrDefault();
                    if (firstFile.ValueKind != JsonValueKind.Undefined)
                        return new LatestVersionInfo { VersionNumber = version.GetProperty("version_number").GetString(), DownloadUrl = firstFile.GetProperty("url").GetString(), FileName = firstFile.GetProperty("filename").GetString() };
                }
            }
            catch { }
            return null;
        ...
        private void RemoveSelectedButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                try { if (File.Exists(item.FilePath)) File.Delete(item.FilePath); _updates.Remove(item); }
                catch { }
            }
        }
        private async void UpdateSelectedButton_Click(object sender, RoutedEventArgs e) =>
            await UpdateItemsAsync(_updates.Where(u => u.IsSelected).ToList());
        ...
        private async Task UpdateItemsAsync(List<UpdateItem> items)
        {
            ...
            foreach (var item in items)
            {
                try
                {
                    if (string.IsNullOrEmpty(item.DownloadUrl) || string.IsNullOrEmpty(item.NewFileName)) { failed++; continue; }
            ...
            await ShowMessageAsync(message, "Update Complete");
            TitleTextBlock.Text = _updates.Count > 0 ? $"{_updates.Count} update(s) available" : "All files are up to date";
        }

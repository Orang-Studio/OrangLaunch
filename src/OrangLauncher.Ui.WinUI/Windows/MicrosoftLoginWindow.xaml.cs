...
namespace OrangLauncher
{
    public sealed partial class MicrosoftLoginWindow : Window
    {
        ...
        private void ProcessUrlButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                // Accept the full redirect URL or a bare pasted code; browsers such as
                // Brave strip ?code= from the address bar shortly after the page loads.
                var code = Managers.MicrosoftAuthManager.TryExtractCodeFromRedirect(url);
                if (!string.IsNullOrEmpty(code))
                {
                ...
                {
                    ShowMessageAsync("Could not extract the authorization code. Copy the address immediately when the blank page appears - the code disappears once the page finishes loading.");
                }
            }
            catch (Exception ex)
            {
